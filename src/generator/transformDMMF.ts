...
export function createTransformer(generatorName: string) {
  ...
  const transformFields = (fields: DMMF.Field[]) => {
    ...
    for (const field of fields.map(transformField)) {
      _fields.push(field.str);
      _inputFields.push(field.strInput);
      for (const d of [...field.deps]) {
        dependencies.add(d);
      }
    }
  ...
  function transformDMMF(dmmf: DMMF.Document) {
    ...
    const generatorHeader =
      '// Generated by prisma-typebox-generator, do not modify.';

    /**
     * Modified by @0xmarko.
     * Adapting this to be used within an Elysia app that already re-exports the typebox library.
     */
    const mainImport = `import { type Static, t as Type } from 'elysia'`;
    ...
    return [
      ...models.map((model) => {
        ...
        return {
          name: model.name,
          rawString: [
            [generatorHeader, mainImport, ...importStatements].join('\n'),
            raw,
      ...
      ...enums.map((enm) => {
        return {
          name: enm.name,
          inputRawString: null,
          rawString: `${generatorHeader}${mainImport}\n\n${transformEnum(enm)}`,
        };
      }),
    ];
  }
